...
public class My_Account extends BaseTest {
	...
	public void My_Account_01_Customer_Info() {
		...
		myAccountPage.enterDOBDropdownInCustomer(birthDay, birthMonth, birthYear);
	...
	public void My_Account_02_Add_Address() {
		...
		myAccountPage.enterToCountryDropdownInAddresses(country);
		myAccountPage.enterToStateProvinceDropdownInAddresses(province);
	...
	public void My_Account_04_My_Product_Reviews() {
		...
		loginPage.clickToProductTitle(driver, productTitle);
		loginPage.clickToMyProductReviewsMenu();
		Assert.assertEquals(loginPage.getProductPageTitle() ,"Product reviews for " + productTitle);

		loginPage.enterToReviewTitleTextbox(reviewTitle);
		loginPage.enterToReviewTextTextbox(reviewText);
		loginPage.selectReviewRatingRadio(ratingRadio);
		loginPage.clickToSubmitReviewButton();
		Assert.assertTrue(loginPage.isReviewResultPageDisplayed());

		loginPage.openHeaderPageByName(driver, "My account");
		myAccountPage = PageGeneratorManager.getMyAccountPage(driver);
		Assert.assertTrue(myAccountPage.isCustomerPageTitleDisplayed());
